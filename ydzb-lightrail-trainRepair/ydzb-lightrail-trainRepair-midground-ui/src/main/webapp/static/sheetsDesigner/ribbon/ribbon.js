(function () {
  ...
  function initRibbonBar() {
      ...
      click: function (evt, cmd) {
        ...
        switch (cmd.commandName) {
          ...
          case "ribbonHidden":
            ...
            // 张天可
            $(".content .fill-spread-content").css({ top: top + 15 })
    ...
    hideSparklineTab(true)
    hideTableTab(true)
    hideFormulaSparklineTab(true)
    hideSlicerTab(true)
    hideChartTab(true)
    hideShapeTab(true)

    // 张天可
    var hideTabIds = [/* "insertTab", */ "formulaTab", "dataTab", "viewTab", "settingTab"]
    var hideGroupIds = ["numbergroup", "celltypegroup", "stylesgroup", "cellsgroup", "editinggroup"]
    if (app.core.isReadOnly) {
      hideGroupIds = hideGroupIds.concat(["clipboardgroup", "fontgroup", "alignmentgroup"])
    }
    if (!app.core.isReadOnly) {
      // 插入只保留图片
      $("#insertTab>ul").children().each(function (i, el) {
        if (el.id != "illustrations") {
          $(el).hide()
        }
      })
    } else {
      hideTabIds.push("insertTab")
    }

    hideTabIds.forEach(function (id) {
      hideTab(id, true)
    })
    hideGroupIds.forEach(function (id) {
      $("#" + id).hide()
    })
  }
  ...
  // 张天可
  function hideTab(id, init) {
    if ($("a[href='#" + id + "']").is(':visible') || init) {
      $(".ribbon-bar").gcuiribbon("setTabPageVisible", id, false)
    }
  }

  function hideSparklineTab(init) {
    if ($("a[href='#sparklineTab']").is(':visible') || init) {
  ...
  function hideFormulaSparklineTab(init) {
    if ($("a[href='#formulaSparklineTab']").is(':visible') || init) {
      $(".ribbon-bar").gcuiribbon("setTabPageVisible", "formulaSparklineTab", false)
    }
  }

  function hideTableTab(init) {
    if ($("a[href='#tableTab']").is(':visible') || init) {
      $(".ribbon-bar").gcuiribbon("setTabPageVisible", "tableTab", false)
    }
  }

  function hideChartTab(init) {
    if ($("a[href='#chartTab']").is(':visible') || init) {
  ...
  function hideShapeTab(init) {
    if ($("a[href='#shapeTab']").is(':visible') || init) {
  ...
  function hideSliderPanel(init) {
    var sliderPanel = $(".slider-panel")
    if ($('.slider-panel-panel2', sliderPanel).is(':visible') || init) {
  ...
  function hideSlicerTab(init) {
    if ($("a[href='#slicerTab']").is(':visible') || init) {
