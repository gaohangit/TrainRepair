(function () {
  ...
  var Menu = (function () {
    function Menu(rootElem) {
      ...
      var this_ = this
      rootElem.find(">li").each(function (i, v) {
        if (!this_.selectedTab && $(v).css("display") !== "none") {
          this_.selectedTab = $(v)
        }
      })
  ...
  var FileMenu = (function () {
    ...
    FileMenu.prototype.initPage = function () {
      // 张天可
      disabledTabs.forEach(function (id) {
        $("[href='#" + id + "']").parent().hide()
      })
      ...
      // 张天可
      if (allTabs.length > 0) {
        $(".menu-title span").text(allTabs[0].show)
        this.mainMenu.showTab("#" + allTabs[0].id)
      }
    ...
    FileMenu.prototype._initAllPages = function () {
      if (!disabledTabs.find(function (v) { return v == "new-page" })) {
        this._initNewPage()
      }
      if (!disabledTabs.find(function (v) { return v == "import-page" })) {
        this._initImportPage()
      }
      if (!disabledTabs.find(function (v) { return v == "export-page" })) {
    ...
    FileMenu.prototype._addExportListeners = function () {
      ...
      $("#export-csv").on('click', function (e) {
        ...
        var sheetIndex = FileMenu._intParser($('#export_csv_sheetIndex').val())
        var row = FileMenu._intParser($('#export_csv_row').val())
        var column = FileMenu._intParser($('#export_csv_column').val())
        var rowCount = FileMenu._intParser($('#export_csv_rowCount').val())
        var columnCount = FileMenu._intParser($('#export_csv_columnCount').val())
        if (row === null) {
          row = 0
        }
        if (column === null) {
        ...
        if (rowCount === null) {
          rowCount = designer.wrapper.spread.sheets[sheetIndex].getRowCount()
        }
        if (columnCount === null) {
          columnCount = designer.wrapper.spread.sheets[sheetIndex].getColumnCount()
        }
  ...
  var allTabs = [
    {
      id: "new-page",
      show: designer.res.fileMenu.new,
    },
    {
      id: "import-page",
      show: designer.res.fileMenu.import,
    },
    {
      id: "export-page",
      show: designer.res.fileMenu.export,
    }
  ]
  var hiddenSelectors = [
    "#import_excel_password",
    "#import-excel-page .sub-title",
    "#import-excel-page>ul",
    "#export_excel_password",
    "#export-excel-page .sub-title",
    "#export-excel-page>ul",
    "#import-page .left-view ul li:nth-child(1)",
    "#import-page .left-view ul li:nth-child(3)",
    "#export-page .left-view ul li:nth-child(1)",
    "#export-page .left-view ul li:nth-child(3)",
    "#export-pdf-page .pdf-export-setting",
  ]
  ...
  designer.loader.ready(function () {
    // 张天可
    disabledTabs.push("new-page")
    if (app.core.isReadOnly) {
      disabledTabs.push("import-page")
    }
    for (var index = 0; index < allTabs.length; index++) {
      const tab = allTabs[index]
      if (disabledTabs.indexOf(tab.id) >= 0) {
        allTabs.splice(index, 1)
        index--
      }
    }
    hiddenSelectors.forEach(function (v) { return $(v).hide() })
