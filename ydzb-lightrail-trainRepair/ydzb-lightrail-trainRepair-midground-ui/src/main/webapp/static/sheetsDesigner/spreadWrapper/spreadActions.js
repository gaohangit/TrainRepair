(function () {
  ...
  function initDesignerCommands(commandManager) {
    ...
    // 张天可
    var customInitCommands = [{
      name: "insertLastRows",
      command: commands_insertLastRows
    }, {
      name: "insertNextRows",
      command: commands_insertNextRows
    }, {
      name: "insertLastColumns",
      command: commands_insertLastColumns
    }, {
      name: "insertNextColumns",
      command: commands_insertNextColumns
    }, {
      name: "deleteColumns",
      command: commands_deleteColumns
    }, {
      name: "deleteRows",
      command: commands_deleteRows
    }]
    customInitCommands.forEach(function (v) {
      var name = ZSJCJYW_COMMAND_PREFIX + v.name
      commandManager.register(name, v.command)
    })
  }
  ...
  var commands_setBorder = {
    ...
    execute: function (context, options, isUndo) {
      ...
      var rs = cmd.execute(context, options, isUndo)
      return rs
    }
  }
  ...
  var MergeCenterAction = (function (_super) {
    ...
    MergeCenterAction.prototype.executeImp = function (context, options, isUndo) {
      ...
      for (var i = 0; i < selections.length; i++) {
        ...
        var col = selection.col === -1 ? 0 : selection.col
        // 张天可
        let result = self._sheet.addSpan(row, col, selection.rowCount, selection.colCount)
        if (result == false) {
          setTimeout(() => {
  ...
  // <-- 张天可
  var CustomInsertRowsAction = (function (_super) {
    designer.extends(CustomInsertRowsAction, _super)
    function CustomInsertRowsAction(spread, options) {
      _super.call(this, spread, options)
    }
    CustomInsertRowsAction.prototype.executeImp = function (context, options, isUndo) {
      var self = this
      var sortedRanges = self.getSortedRowSelections()
      if (sortedRanges.length != 1 || sortedRanges[0].rowCount != 1) {
        console.error("此操作只能插入一行")
        return
      }
      var rowIndex = sortedRanges[0].row
      if (options.insertType == "last") {
        self._sheet.addRows(rowIndex, 1, rowIndex)
      } else if (options.insertType == "next") {
        self._sheet.addRows(rowIndex + 1, 1, rowIndex)
      }
      designer.util.setSpanStyleWhenInsertRowsColumns(self._sheet, rowIndex, -1, 1)
    }
    return CustomInsertRowsAction
  })(InsertRowsAction)
  var commands_insertLastRows = {
    canUndo: true,
    execute: function (context, options, isUndo) {
      options.considerAllSheets = true
      options.insertType = "last"
      var cmd = new CustomInsertRowsAction(context, options)
      return cmd.execute(context, options, isUndo)
    }
  }
  function insertLastRows(spread, paramters) {
    runCommand(spread, "insertLastRows", commands_insertLastRows, {
      selections: paramters.selections,
      sheetName: paramters.sheetName
    })
  }
  spreadActions.insertLastRows = insertLastRows

  var commands_insertNextRows = {
    canUndo: true,
    execute: function (context, options, isUndo) {
      options.considerAllSheets = true
      options.insertType = "next"
      var cmd = new CustomInsertRowsAction(context, options)
      return cmd.execute(context, options, isUndo)
    }
  }
  function insertNextRows(spread, paramters) {
    runCommand(spread, "insertNextRows", commands_insertNextRows, {
      selections: paramters.selections,
      sheetName: paramters.sheetName
    })
  }
  spreadActions.insertNextRows = insertNextRows
  // 张天可 -->
  ...
  // <-- 张天可
  var CustomInsertColumnsAction = (function (_super) {
    designer.extends(CustomInsertColumnsAction, _super)

    function CustomInsertColumnsAction(spread, options) {
      _super.call(this, spread, options)
    }

    CustomInsertColumnsAction.prototype.executeImp = function (context, options, isUndo) {
      var self = this
      var sortedRanges = self.getSortedColumnSelections()
      if (sortedRanges.length != 1 || sortedRanges[0].colCount != 1) {
        console.error("此操作只能插入一列")
        return
      }
      var colIndex = sortedRanges[0].col
      if (options.insertType == "last") {
        self._sheet.addColumns(colIndex, 1, colIndex)
      } else if (options.insertType == "next") {
        self._sheet.addColumns(colIndex + 1, 1, colIndex)
      }
      designer.util.setSpanStyleWhenInsertRowsColumns(self._sheet, -1, colIndex, 1)
    }
    return CustomInsertColumnsAction
  })(InsertColumnsAction)
  var commands_insertLastColumns = {
    canUndo: true,
    execute: function (context, options, isUndo) {
      options.considerAllSheets = true
      options.insertType = "last"
      var cmd = new CustomInsertColumnsAction(context, options)
      return cmd.execute(context, options, isUndo)
    }
  }
  function insertLastColumns(spread, paramters) {
    runCommand(spread, "insertLastColumns", commands_insertLastColumns, {
      selections: paramters.selections,
      sheetName: paramters.sheetName
    })
  }
  spreadActions.insertLastColumns = insertLastColumns

  var commands_insertNextColumns = {
    canUndo: true,
    execute: function (context, options, isUndo) {
      options.considerAllSheets = true
      options.insertType = "next"
  ...
  function insertNextColumns(spread, paramters) {
    runCommand(spread, "insertNextColumns", commands_insertNextColumns, {
      selections: paramters.selections,
      sheetName: paramters.sheetName
    })
  }
  spreadActions.insertNextColumns = insertNextColumns
  //  张天可 -->
