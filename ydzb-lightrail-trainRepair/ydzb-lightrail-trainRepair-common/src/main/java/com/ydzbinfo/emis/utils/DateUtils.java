...
/**
 * 此工具类已过期，请使用DataTimeUtil工具类，日期、时间的计算请使用java8新增的LocalDateTime、LocalDate和LocalTime及其相关工具类
 * by 张天可
 *
 * @author dfg 日期工具类
 */
...
public class DateUtils {
	 ...
	 * @return 返回yyyy-MM-dd HH:mm格式的时间字符串
	 */
	public static String dateTime24ToStr(Date date) {
		return DateUtils.dateToFormatStr(date, "yyyy-MM-dd HH:mm:ss");
	}
	...
	/** 获取两个时间的时间查 如1天2小时30分钟 */
	public static String getDatePoor1(Date endDate, Date nowDate) {
...
//        long hour = -(diff % nd / nh);
	...
	/** 获取两个时间的时间查 如1天2小时30分钟 */
	public static String getDatePoor3(Date endDate, Date nowDate) {
	...
	/** 获取两个时间的时间差 差30分钟 */
	public static String getDatePoor4(Date endDate, Date nowDate) {
		long nd = 1000 * 24 * 60 * 60;
		long nh = 1000 * 60 * 60;
		long nm = 1000 * 60;
		long ns = 1000;
	...
	public static Date getLashMonth(int subDate, boolean flag) {
		...
		if (flag) {
			...
			result = DateUtils.getDateBegin(cal.getTime());
		} else {
			Calendar cal = Calendar.getInstance();
			if (subDate != 0) {
			...
			result = DateUtils.getDateEnd(cal.getTime());
		}
		try {
	...
	public static List<String> getBetweenDate(String startTime, String endTime){
		...
		try {
			...
			while (startDate.getTime() <= endDate.getTime()){
				list.addAll(Collections.singleton(sdf.format(startDate)));
